...
class LoginScreenState extends State<SignUpMainScreen> {
  ...
  Future<void> sendVerificationCode() async {
    ...
    if (referralController.text.trim().isNotEmpty) {
      try {
        var response = await checkReferralCode(
        ...
        if (response['error'] == true) {
          ...
          HelperUtils.showSnackBarMessage(context, response['message']);
          HiveUtils.setReferralCode(referralController.text.toString());
        }
      } catch (e) {
          ...
          children: [
            ...
            if (Constant.appleAuthentication == "1" && Platform.isIOS)
              UiUtils.buildButton(context,
                  ...
                  buttonColor: secondaryColor_,
                  border: context.watch<AppThemeCubit>().state.appTheme != AppTheme.dark
                      ? BorderSide(color: context.color.textDefaultColor.withValues(alpha: 0.5))
                      : null,
